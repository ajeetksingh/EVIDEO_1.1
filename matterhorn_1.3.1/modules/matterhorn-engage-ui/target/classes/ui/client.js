...
$(document).ready(function(){
	...
	$.getJSON("/info/me.json", function(r){
				r=JSON.stringify(r);
	...
	if (r==true)
  	{
		...
		xmlhttp.open("GET","http://localhost:81/json-delete-bookmark.php?"+str+'&jsoncallback=?',true);
		xmlhttp.send();
...
function testTesting()
{
		...
		xmlhttp.onreadystatechange=function()
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
 		 	  document.getElementById("facebook").innerHTML=xmlhttp.responseText;
   		 	}
		}
		xmlhttp.open("GET","../php/PhpExecutorService?name=udit",true);
		xmlhttp.send();
//		popup('popUpDeleteBookmark');
	...
	if (r==true)
  	{
		...
		xmlhttp.onreadystatechange=function()
			...
			xmlhttp.open("GET","http://localhost:81/json-delete-link.php?"+str+'&jsoncallback=?',true);
			xmlhttp.send();
			popup('popUpDiv5');
		...
		if (r==true)
	  	{
			...
			xmlhttp.open("GET","http://localhost:81/json-delete-annotation.php?"+str+'&jsoncallback=?',true);
			xmlhttp.send();
			popup('popUpDiv6');
		}
}
...
// Function to Retrieve the Annotation from database for Editing
	...
	function editAnnotation(id)
	{
			...
			var titleid;
			var str= "id="+id+"&media_id="+mid;
 			$.getJSON('http://localhost:81/json-retrieve-annotation.php?'+str+'&jsoncallback=?', function(r) {
...
function seekVideoPage(id)
{
		...
		var user=readCookie('sessionId');
		 var str= "id="+id+"&user="+user;
 				 $.getJSON('http://localhost:81/json-retrieve-bookmark-nextid.php?'+str+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
			//alert(r);
			r=JSON.stringify(r);
			var temp =JSON.parse(r);
			...
			if(media_id)
			{
			window.location = "http://172.18.1.15:8080/engage/ui/watch.html?id="+media_id+"&start="+start+"&stop="+stop+"&bid="+bid;
                      }
			else
			{
...
function seekVideo(id)
{
		...
		var media_id;
		 var str= "id="+id;
 				 $.getJSON('http://localhost:81/json-retrieve-bookmark.php?'+str+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
			//alert(r);
			r=JSON.stringify(r);
			var temp =JSON.parse(r);
			...
			if(media_id)
			{
			window.location = "http://172.18.1.15:8080/engage/ui/watch.html?id="+media_id+"&start="+start+"&stop="+stop+"&bid="+id;
                      }
			else
			{
				alert("End of Bookmarks");
...
function seekVideoForBookmark(id)
{
		...
		var media_id;
 		...
 		console.log("Inside getJSON");
			...
			if(media_id)
			{
			window.location = "http://172.18.1.15:8080/engage/ui/watch.html?id="+media_id+"&start="+start+"&bid="+id+"&run="+runCommand;
                      }
...
function editBookmark(id)
				{
					...
					//alert(str);
				        $.getJSON('http://localhost:81/get-bookmark-data.php?'+str+'&jsoncallback=?', function(r) {
 					console.log("Inside getJSON");
                      			r=JSON.stringify(r);
                       			var temp =JSON.parse(r);
                     			console.log(temp[0].title);
...
function editLink(id)
				{
					...
					//alert(str);
				        $.getJSON('http://localhost:81/get-link-data.php?'+str+'&jsoncallback=?', function(r) {
 					console.log("Inside getJSON");
                      			r=JSON.stringify(r);
                       			var temp =JSON.parse(r);
                     			console.log(temp[0].title);
...

		{
			...
			var str="id="+id;
			//alert(str);
			$.getJSON('http://localhost:81/BookmarkDetails.php?'+str+'&jsoncallback=?', function(r) {
 			console.log("Inside getJSON");
                      	r=JSON.stringify(r);
                       	var temp =JSON.parse(r);
                     	console.log(temp[0].title);
		...
		function vwAnnDetails(id)
		{
			...
			$.getJSON('http://localhost:81/json-create-Annotation.php?media_id='+id+'&jsoncallback=?', function(r) {
	...
	function updateLink()
	{
		var xmlhttp;
		var mid=QueryString.id;
		var mstart=parseFloat(document.formlinkedit.start1.value);
		var mstop=parseFloat(document.formlinkedit.stop1.value);
				...
				if(!isNaN(mstart)&&!isNaN(mstop))
				{
					if(mstart>0 && mstop>0)
					{
						if(mstart<mstop)
						{
							...
							xmlhttp.open("GET","http://localhost:81/update-link-data.php?"+str+'&jsoncallback=?',true);
							xmlhttp.send();
	...
	function updateBookmark()
	{
		...
		var xmlhttp;
	        var mid=QueryString.id;
		var mstart=parseFloat(document.formbookmarkedit.start1.value);
		var mstop=parseFloat(document.formbookmarkedit.stop1.value);
			...
			if(!isNaN(mstart)&&!isNaN(mstop))
			{
				if(mstart>0 && mstop>0)
				{
					if(mstart<mstop)
					{
						...
						xmlhttp.open("GET","http://localhost:81/update-bookmark-data.php?"+str+'&jsoncallback=?',true);
						xmlhttp.send();
						popup('popUpBookmarkEdit');
 ...
 var refreshId = setInterval(function(){
			...
			var stime = Opencast.Player.getCurrentPosition();
			var id = $.getURLParameter('id');
			$.getJSON('http://localhost:81/dynAnnotation.php?id='+id+'&stime='+stime+'&jsoncallback=?', function(r) {
				console.log("Inside getJSON for Annotation Show from Advanced Results");
				//alert(r);
				r=JSON.stringify(r);
				//alert(r);
				...
				for (i=0;i<length1;i++)
				{
					if(temp[i].annotations){
						$('#annotationCurrent').text(temp[i].annotations);
					}
					else
					{
						$('#annotationCurrent').text("No Annotation for the current Segment");
					}
					console.log(temp[i].annotations);
				}
				//console.log(data);
			});
...
function openBookmark()
{
	...
	document.formbookmarkadd.username.value=user_name;
...
(function ($) {
    ...
    // if (local) {
    //     $('head').append('<link rel="stylesheet" href="http://localhost:81/style/default.css" type="text/css" />');
    // } else {
        style = 'REPLACE_WITH_STYLE'; //todo: automate this
        style = '._vp_widget{ width:400px;} .vp_header{text-align:right; background-color:#FDE7CE; border:1px solid gray; text-align:center}.vp_footer{ width:100%; text-align:right; background-color :#FDE7CE; border:1px solid gray}.vp_footer a{margin-right:5px;color:#A01A1A}.vp_list{ float:left; text-align:left; border:1px solid gray; background-color:white; width:100%; overflow:auto; position: relative}.vp_lst_even{background-color:white;padding:4px}.vp_lst_odd{background-color:#FDE7CE;padding:4px;}.vp_lst_active{background-color:#787676}.vp_time{ position:absolute; margin:0 3px 0 0; right:0px; background-color:gray; width:45px; text-align:center; border:1px solid darkgrey; color:ghostwhite; text-decoration:none}';
	...
	var value = new Array(v.id, v.time, v.title);
        var div = $('<div>').append($('<a>', {
            ...
            html: $('<span> <span class="vp_fns"></span><a style="float:right;color:#A01A1A;" href="javascript:editAnnotation('+v.id+');"><img src="http://localhost:81/edit.png" title="Edit"></a><a style="float:right;color:#ffffff" href="javascript:deleteAnnotation('+v.id+');"><img src="http://localhost:81/Delete.png" title="Delete"></a><a style="float:right;" href="javascript:appendAnnotation('+v.id+');"><img title="Add More"src="http://localhost:81/add.png"></a><p style="text-align:justify; width:23em;"><span class="vp_title" >'+v.title+'</span></span>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        }).append($('<span>', {
            'class': 'vp_time',
    ...
    function make_title1(v) {
        ...
        var div = $('<div>').append($('<a>', {
            ...
            html: $('<span> <span class="vp_fns"></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<a href="'+v.url+'" style="color:blue;" target="_blank">[View More]</a><a style="float:right;color:#A01A1A;" href="javascript:abc2('+v.id+');"><img src="http://localhost:81/edit.png" title="Edit"></a><a style="float:right;color:#ffffff;" href="javascript:deleteLink('+v.id+');"><img src="http://localhost:81/Delete.png" title="Delete"></a></span></span><p>'+v.description+'</p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        }).append($('<span>', {
            'class': 'vp_time',
            html: sec2norm(v.time)
        }))).css({'text-decoration':'none'}).data({
     ...
     function make_title2(v) {
        ...
        var div = $('<div>').append($('<a>', {
            ...
            html: $('<span> <span class="vp_fns"></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<a style="float:right;color:#A01A1A;" href="javascript:abc3('+v.id+');"><img src="http://localhost:81/edit.png" title="Edit"></a><a style="float:right;color:#ffffff;" href="javascript:deleteBookmark('+v.id+');"><img src="http://localhost:81/Delete.png" title="Delete"><a style="float:right;color:#ffffff;" href="javascript:showBookmarkDetails(&#39;'+v.media_id+'&#39;);"><img src="http://localhost:81/play/details-icon.png" title="Details"></a></a></span></span><p>'+v.desc+'</p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        }).append($('<span>', {
            'class': 'vp_time',
	...
	if(i==true)
	{
		...
		console.log("In Annotations Representation");
        	var div = $('<div style="background:#'+hext+';">').append($('<a>', {
            	id    : ('vp_id_'+v.time).replace('.', '_'),
            	href  : 'http://172.18.1.15:8080/engage/ui/watch.html?id='+v.media_id+'&start='+v.start+'&q='+title1+'&cat='+categ1
        	}).append($('<span>', {
            	html: $('<span> <span class="vp_fns"><img src="http://localhost:81/frames/annotations/'+v.media_id+v.start+'.png" title = "'+v.title+'" height = "75px" align="left" ></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<span style="font-weight:normal;float:right"><a href=javascript:viewAnnDetails(&#39;'+v.media_id+'&#39;><img src="http://localhost:81/anno.png" title="Annotations"></a><a href=javascript:viewOCRDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/ocr.png" title="OCR Text"></a></span></span></span><p><div><table><tr><td style="font-weight:bold;">Name:</td><td>'+v.media_name+'</td></tr><tr><td style="font-weight:bold;">Author:</td><td>'+v.media_creator+'</td></tr><tr><td style="font-weight:bold;">Duration</td><td>'+v.duration+'</td></tr><tr><td style="font-weight:bold;">Explanation:</td><td>'+v.description+'</td></tr></table></div></p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        	}).append($('<span>', {
            	'class': 'vp_time',
            	html: sec2norm(v.start)+"-"+sec2norm(v.stop)
        	}))).css({'text-decoration':'none'}).data({
		...
		console.log("In link Representation");
        	...
        	var div = $('<div style="background:#'+hext+';">').append($('<a>', {
        	    ...
        	    html: $('<span> <span class="vp_fns"></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<a href='+v.url+' style="color:blue;">[View More]</a><span style="font-weight:normal;float:right"><a href=javascript:viewAnnDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/anno.png" title="Annotation"></a><a href=javascript:viewOCRDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/ocr.png" title="OCR Text"></a></span></span></span><p><div><table><tr><td style="font-weight:bold;">Name:</td><td>'+v.media_name+'</td></tr><tr><td style="font-weight:bold;">Author:</td><td>'+v.media_creator+'</td></tr><tr><td style="font-weight:bold;">Duration</td><td>'+v.duration+'</td></tr><tr><td style="font-weight:bold;">Explanation:</td><td>'+v.description+'</td></tr></table></div></p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        	}).append($('<span>', {
        	    'class': 'vp_time',
        	    html: sec2norm(v.start)+"-"+sec2norm(v.stop)
        	}))).css({'text-decoration':'none'}).data({
		...
		$('"#'+vp_bid+'"').hide();
        	var div = $('<div style="background:#'+hext+';">').append($('<a>', {
        	    ...
        	    html: $('<span> <span class="vp_fns"></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<span style="font-weight:normal;float:right"><a href=javascript:viewAnnDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/anno.png" title="Annotation"></a><a href=javascript:viewOCRDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/ocr.png" title="OCR Text"></a></span></span></span><p id='+vp_bid+'><div><table><tr><td style="font-weight:bold;">Name:</td><td>'+v.media_name+'</td></tr><tr><td style="font-weight:bold;">Author:</td><td>'+v.media_creator+'</td></tr><tr><td style="font-weight:bold;">Duration</td><td>'+v.duration+'</td></tr><tr><td style="font-weight:bold;">Explanation:</td><td>'+v.description+'</td></tr></table></div></p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        	}).append($('<span>', {
        	    'class': 'vp_time',
        	    html: sec2norm(v.start)+"-"+sec2norm(v.stop)
        	}))).css({'text-decoration':'none'}).data({
	...
	else
	{
		...
		console.log("In Annotations Representation");
        	var div = $('<div style="background:#'+white+';">').append($('<a>', {
            	...
            	html: $('<span> <span class="vp_fns"><img src="http://localhost:81/frames/annotations/'+v.media_id+v.start+'.png" title = "'+v.title+'" height = "75px" align="left" ></span>'+'<span class="vp_title" style="font-weight:bold;">'+v.title+'<span style="font-weight:normal;float:right"><a href=javascript:viewAnnDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/anno.png" title="Annotation"></a><a href=javascript:viewOCRDetails(&#39;'+v.media_id+'&#39;)><img src="http://localhost:81/ocr.png" title="OCR Text"></a></span></span></span><p><div><table><tr><td style="font-weight:bold;">Name:</td><td>'+v.media_name+'</td></tr><tr><td style="font-weight:bold;">Author:</td><td>'+v.media_creator+'</td></tr><tr><td style="font-weight:bold;">Duration</td><td>'+v.duration+'</td></tr><tr><td style="font-weight:bold;">Explanation:</td><td>'+v.description+'</td></tr></table></div></p>').addClass( v.childrens ? 'expand sublist' : v.top ? 'vp_sub_list' : 'vp_entry')
        	}).append($('<span>', {
            	'class': 'vp_time',
            	html: sec2norm(v.start)+"-"+sec2norm(v.stop)
        	}))).css({'text-decoration':'none'}).data({
	...
	//edited_Devanshu
        create_header: function (str) {
            return $('<div>', {
                id: 'vp_header',
                'class': 'vp_header',
                'html': '<a style="float:left;color:#ffffff;display:inline-block;" class="button1" href="javascript:abc()"><span>Add '+str+'</span></a><span><a style="color:#A01A1A;float:left;display:inline-block;" href="http://medialab.iitj.ac.in">'+str+'</a></span><a id="closeAnn" style="margin-left:125px;color:#ffffff;display:inline-block;" class="button1">Close</a>'
	...
	create_header1: function (str) {
            return $('<div>', {
                id: 'vp_header',
                'class': 'vp_header',
                'html': '<a class="button1" style="float:left;color:#ffffff;" href="javascript:abc1()"><span>Add '+str+'</span></a>&nbsp;&nbsp;&nbsp;<a style="color:#A01A1A" href="http://medialab.iitj.ac.in">'+str+'</a>&nbsp;&nbsp;<img id="closeLink" style="margin-left:125px;cursor:pointer;" src="close_button.png">'
	...
	create_header2: function (str) {
            return $('<div>', {
                id: 'vp_header',
                'class': 'vp_header',
                'html': '<a class="button1" style="float:left;color:#ffffff;" href="javascript:openBookmark()"><span>Add '+str+'</span></a>&nbsp;&nbsp;<a class="button1" style="float:left;color:#ffffff;" href="javascript:playlist()"><span>View All</span></a><a style="color:#A01A1A" href="http://medialab.iitj.ac.in">'+str+'</a>&nbsp;&nbsp;<a id="closeBookmark" style="margin-left:100px;cursor:pointer;color:#ffffff;" class="button1">Close</a>'
	...
	create_header3: function (str) {
            return $('<div>', {
                id: 'vp_header',
                'class': 'vp_header',
                'html': '<a class="button1" style="float:left;color:#ffffff;" href="javascript:openBookmark()"><span>Add '+str+'</span></a>&nbsp;&nbsp;<a class="button1" style="float:left;color:#ffffff;" href="javascript:playlist()"><span>View All</span></a><a style="color:#A01A1A" href="http://medialab.iitj.ac.in">'+str+'</a>&nbsp;&nbsp;<a id="close" style="margin-left:100px;cursor:pointer;color:#ffffff;" class="button1">Close</a>'
        ...
        create_list: function (w) {
            var res= $('<div>', {
                id: 'vp_list',
                'class': 'vp_list'
            }).css('height', Math.round(w*5/8) + 'px');
            return res;
        }
    };
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/*function getMetaData(categ,title,user)
	{

		var r;
		console.log("Inside getMetaData");
		alert(categ+" "+title+" "+user);
		$.getJSON('http://localhost/search_data.php?title='+title+'&run='+categ+'&user='+user+'&jsoncallback=?',function(r){console.log("Inside getTerms"); });
			//console.log("Inside getJSON of getMetadata");
...
function create_sidebar3(data,str,title,categ,user) {
	...
	if(categ!="all")
	{
		...
		$.getJSON('http://localhost:81/read-xml-trans.php?title='+title+'&user='+user_name+'jsoncallback=?', function(r4){
	...
	{
				var load = [ { url: 'http://localhost:81/search_data.php?title=' + title + '&run=annotations&jsoncallback=?', before: null },
    						{ url: 'http://localhost:81/search_data.php?title=' + title + '&run=Link&jsoncallback=?', before: null },
    						{ url: 'http://localhost:81/search_data.php?title=' + title + '&user=' + user_name + '&run=bookmarks&jsoncallback=?', before: null },
   						 { url: 'http://localhost:81/read-xml-trans.php?title=' + title + '&jsoncallback=?', before: function() { $vpl.append('<div>Related Terms</div>'); } }
						];
...
///////////////////////////////////////////////////////////////////////////
	...
	$vp = get_widget_div();
        ...
        console.log('width: ', $vp.css('width'));
	...
	$footer.css('width',parseInt(h/4.5));
	$header.css('width',parseInt(h/4.5));
	console.log("adshfladjfkjadfjkajwidth: "+w);
    ...
    vp.load_app = function (str) {
		 ...
		 $.getJSON('http://localhost:81/json-create-Annotation.php?media_id='+videoid+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
                      console.log(JSON.stringify(r));
...
vp1.load_app1 = function (str) {
        console.log("In load App");
		 $.getJSON('http://localhost:81/json-create.php?media_id='+videoid+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
                      console.log(JSON.stringify(r));
                      //create_sidebar(r,str);
			create_sidebar1(r,str);
...
vp2.load_app2 = function (str) {
        console.log("In load App");
		 $.getJSON('http://localhost:81/json-get-bookmark.php?user='+user_name+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
                      console.log(JSON.stringify(r));
                      //create_sidebar(r,str);
...
vp3.load_app3 = function (str) {
		...
		if(QueryString.q!=undefined && QueryString.cat!=undefined)
		{
		 ...
		 $.getJSON('http://localhost:81/search_data.php?user='+user_name+'&title='+title+'&run='+categ+'&jsoncallback=?', function(r) {
 		console.log("Inside getJSON");
                      console.log(JSON.stringify(r));
                      //create_sidebar(r,str);
			var count=r.length;
			create_sidebar3(r,str,title,categ,user_name);
   		//console.log(JSON.stringify(data));
	...
	$.getJSON('http://localhost:81/search_data.php?user='+user_name+'&title='+v+'&run=annotations&jsoncallback=?', function(r){
		console.log("Inside Related Search Terms");
		create_sidebar3(r,"Search",v,"all",user_name);
	});
