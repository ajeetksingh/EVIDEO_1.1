...
public class IngestRestService {
  ...
  protected void insertMediaInfo(MediaPackage mp){
      ...
      try{
          ...
          if ( mp == null ){
               ...
               logger.error("MP is null!");
          
          } else {
          
               logger.info("MP is fine!");
          
          }
          
          //MediaInfo media = new MediaInfo();
          
          logger.info("Media ID to be uploaded is "+mp.getIdentifier());
          //media.setMediaId(mp.getIdentifier().compact());
          
          logger.info("Media Duration is "+mp.getDuration());
          long millis = mp.getDuration();
          String hms = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(millis),
            TimeUnit.MILLISECONDS.toMinutes(millis) - TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(millis)),
            TimeUnit.MILLISECONDS.toSeconds(millis) - TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(millis)));
          //media.setDuration(hms);
          
          //logger.info("Media Author is "+mp.getCreators());
          //media.setMediaCreator(mp.getCreators()[0]);
          
         // media.setMediaCreator("Hell Yeah!!");
          
          logger.info("Media Title is "+mp.getTitle());
          //media.setMediaName(mp.getTitle());
          
          //logger.info("Media Uploader is "+user.getUserName());
          //media.setUploader(user.getUserName());
          
          //em.persist(media);
         // tx.commit();
          logger.info("Media Info for MediaID "+mp.getIdentifier().compact()+" Updated in The Database");
          //em.close();
          
      }catch(Exception e){
          
          logger.error("Error while Updating the Media Inof "+e);
          //tx.rollback();
